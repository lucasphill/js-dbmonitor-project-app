...
O workflow [release.yml](.github/workflows/release.yml) roda em cada push para `main` e também pode ser iniciado manualmente. Se `v<versão>` do `package.json` ainda não existir, ele valida e empacota Windows e Linux, cria a tag e publica uma GitHub Release com `.exe`, `.AppImage` e `.deb`. Se a versão já tiver tag, pushes seguintes não repetem a publicação; atualize e envie a versão no `package.json` para gerar outra release. Os artefatos dos workflows de build ficam em **Actions** por 14 dias; os instaladores publicados ficam em **Releases**. A área **Packages** do GitHub é para registries como npm e contêineres, não recebe automaticamente os instaladores Electron. A release não assina os pacotes e não usa segredos de AWS ou do banco. Testes que exigem PostgreSQL real são ignorados sem `BDASH_TEST_PG`. A pasta `release/` fica fora do Git.
...
| Inventário e tamanho dos bancos | `pg_database`, `pg_stat_database`, `pg_database_size` | Metadados e conexões atuais são consultados ao abrir a tela Bancos. Busca, filtros e ordenação são aplicados antes da paginação. Normalmente o tamanho é calculado somente para a página visível; ao ordenar por tamanho, o PostgreSQL precisa medi-lo para todos os bancos filtrados. Pode ficar indisponível sem privilégio `CONNECT` ou se a consulta exceder o tempo limite. |
