...
function DatabaseTable({ block }: { block: DataBlock<DatabaseSummary[]> }) {
  ...
  return <Card className="min-w-0">
    ...
    <CardContent className="flex flex-col gap-4">
      {showRows ? <div className="overflow-x-auto rounded-md border"><Table>
        <TableHeader><TableRow><TableHead>Banco</TableHead><TableHead className="text-right">Conexões</TableHead><TableHead className="text-right">Commits</TableHead><TableHead className="text-right">Rollbacks</TableHead><TableHead className="text-right">Cache hit</TableHead></TableRow></TableHeader>
...
export default function Home() {
  ...
  const { overview, loading, error, refresh } = useDashboard(period, source)
  ...
  return <DashboardShell section={section} onSectionChange={setSection} instance={overview?.instance} refreshing={loading || manualRefreshing || switching} onRefresh={() => void refreshNow()} profiles={profiles} source={source} switching={switching || profileLoading} onSelectProfile={(id) => void selectProfile(id)}>
    {profileError ? <p role="alert" className="rounded-lg border border-destructive/40 p-3 text-sm text-destructive">{profileError}</p> : null}
    {profileLoading ? <div role="status" className="rounded-lg border p-6 text-sm text-muted-foreground">Carregando origens PostgreSQL…</div> : !source && typeof window !== "undefined" && window.bdash ? <div role="alert" className="rounded-lg border p-6 text-sm text-destructive">Nenhuma origem ativa disponível. {profileError}</div> : section === "overview" ? <>
      ...
      {overview ? <>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {metricTitles.map((title, index) => {
            ...
            return <MetricCard key={title} title={title} metric={metric} block={metricBlock(overview.metrics, title, metric, overview.capabilities.statements.available, overview.capabilities.statements.reason)} icon={metricIcons[index]} />
          })}
        </div>
        <OverviewCharts connections={overview.connectionsSeries} transactions={overview.transactionsSeries} />
        <DatabaseTable block={overview.databases} />
    ...
    </> : section === "connections" ? <ConnectionsView key={`connections-${source?.profileId}-${source?.generation}`} /> : section === "performance" ? <PerformanceView key={`performance-${source?.profileId}-${source?.generation}`} /> : section === "databases" ? <DatabasesView key={`databases-${source?.profileId}-${source?.generation}`} /> : section === "logs" ? <LogsView key={`logs-${source?.profileId}-${source?.generation}`} onConfigure={() => setSection("settings")} /> : section === "settings" ? <DiagnosticsView key={`settings-${source?.profileId}-${source?.generation}`} profiles={profiles} source={source} onProfilesChanged={reloadProfiles} onSelectProfile={selectProfile} /> : <div className="flex min-h-80 flex-col items-start justify-center gap-3 rounded-xl border bg-card p-8">
